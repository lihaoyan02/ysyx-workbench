...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 001 ????? 01000 11", sj     , S, Mw(src1 + imm, 2, src2));
	...
	INSTPAT("0000001 ????? ????? 110 ????? 01100 11", rem 	 , R, R(rd) = (src1==1<<31 && src2==-1) ? 0 : (src2==0) ? src1 : (signed)src1 % (signed)src2); 
	INSTPAT("0000001 ????? ????? 111 ????? 01100 11", remu 	 , R, R(rd) = (src2==0) ? src1 : src1 % src2); 
	INSTPAT("0000001 ????? ????? 100 ????? 01100 11", div 	 , R, R(rd) = (src1==1<<31 && src2==-1) ? src1 : (src2==0) ? -1 : (signed)src1 / (signed)src2); 
	...
	INSTPAT("0000001 ????? ????? 010 ????? 01100 11", mulhsu , R, R(rd) = BITS((int64_t)(int32_t)src1 * (uint64_t)src2, 63, 32)); 
	INSTPAT("0000001 ????? ????? 011 ????? 01100 11", mulhu  , R, R(rd) = BITS((uint64_t)src1 * (uint64_t)src2, 63, 32)); 

  INSTPAT("0000000 00001 00000 000 00000 11100 11", ebreak , N, NEMUTRAP(s->pc, R(10))); // R(10) is $a0
  INSTPAT("0000000 00000 00000 000 00000 11100 11", ecall  , N, s->dnpc = isa_raise_intr(0x1, s->pc)); // R(10) is $a0
  INSTPAT("0011000 00010 00000 000 00000 11100 11", mret   , N, s->dnpc = SR(0)); // R(10) is $a0
  INSTPAT("??????? ????? ????? 001 ????? 11100 11", csrrw  , N, 
			...
			switch(BITS(s->isa.inst, 31, 20)) {
				...
				case 0x305: srv = SR(3); SR(3) = src1; break; //mtvec
				default: INV(s->pc);
			}
			R(rd) = srv;
