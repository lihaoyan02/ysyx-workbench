...
static int parse_args(int argc, char *argv[]) {
  const struct option table[] = {
    ...
    {"pcfile"   , required_argument, NULL, 'c'},
  ...
  while ( (o = getopt_long(argc, argv, "-bhl:d:p:e:", table, NULL)) != -1) {
    switch (o) {
      ...
      case 'c': pc_log_file = optarg; break;
      ...
      default:
        ...
        printf("\t-c,--pcfile=FILE        record executed PC values in binary FILE\n");
...
void init_monitor(int argc, char *argv[]) {
  ...
  /* Initialize PC tracing. */
  init_pc_log(pc_log_file);
